...
class Cohort:
    ...
    def __init__(self, name: str, strategy: CohortStrategy, max_agents: int = 10):
        ...
        self._leader_id: str | None = None  # For leader-follower
    ...
    async def execute_task(self, task: dict[str, Any], state: StateManager) -> Any:
    ...
    async def _execute_round_robin(self, task: dict[str, Any], state: dict[str, Any]) -> Any:
        ...
        return await agent.run(input_data=task.get("input"), state=state, tools=task.get("tools"))

    async def _execute_load_balanced(self, task: dict[str, Any], state: dict[str, Any]) -> Any:
        ...
        return await least_busy_agent.run(
            input_data=task.get("input"), state=state, tools=task.get("tools")
        )

    async def _execute_broadcast(self, task: dict[str, Any], state: dict[str, Any]) -> list[Any]:
        ...
        tasks = [
            agent.run(input_data=task.get("input"), state=state, tools=task.get("tools"))
            for agent in self._agents.values()
        ]
    ...
    async def _execute_leader_follower(self, task: dict[str, Any], state: dict[str, Any]) -> Any:
        ...
        return await leader.run(input_data=task.get("input"), state=state, tools=task.get("tools"))
