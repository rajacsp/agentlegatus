...
class Agent:
    ...
    async def run(
        ...
        try:
            ...
            result = await self.provider.execute_agent(self._agent_instance, input_data, state)
    ...
    async def invoke_tool(
        self, tool_name: str, tool_input: dict[str, Any], context: dict[str, Any] | None = None
    ) -> Any:
        ...
        if AgentCapability.TOOL_USE not in self.capabilities:
            raise ValueError(f"Agent {self.agent_id} does not have TOOL_USE capability")
    ...
    async def store_memory(
        ...
        if AgentCapability.MEMORY not in self.capabilities:
            raise ValueError(f"Agent {self.agent_id} does not have MEMORY capability")
    ...
    async def retrieve_memory(
        self, query: str, memory_type: str = "short_term", limit: int = 10
    ) -> list[Any]:
        ...
        if memory_type == "short_term":
            ...
            return await self.memory_manager.get_recent(MemoryType.SHORT_TERM, limit)
        elif memory_type == "long_term":
            # Use semantic search for long-term memory
            return await self.memory_manager.semantic_search(query, limit=limit)
        else:
            raise ValueError(f"Unknown memory type: {memory_type}")
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        """
        avg_time = self._total_execution_time / self._task_count if self._task_count > 0 else 0.0
